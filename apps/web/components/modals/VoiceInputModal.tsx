...
export const VoiceInputModal: React.FC<VoiceInputModalProps> = ({ isOpen, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-md animate-in fade-in duration-200">
      <div className="relative w-full max-w-md rounded-3xl bg-slate-900 text-slate-100 shadow-2xl border border-slate-800 p-6 flex flex-col items-center text-center overflow-hidden">
        {/* 背景裝飾光暈 */}
        ...
        <div className="mt-4 min-h-[48px] flex flex-col items-center justify-center">
          {isListening ? (
            <p className="text-xs font-bold text-emerald-400 flex items-center gap-1.5">
              ...
              <span>正在聆聽中... 請直接說話（例如：「中午吃拉麵 280 街口」）</span>
            </p>
          ) : isParsing ? (
          ...
          {/* 即時語音文字氣泡 */}
          {transcript && (
            <div className="mt-3 px-4 py-2 rounded-2xl bg-slate-800/80 border border-slate-700 max-w-xs text-xs text-slate-200 font-medium">
              「{transcript}」
            </div>
          )}
        </div>
        ...
        {/* AI 解析完成的結構化確認預覽卡片 */}
        {parsedResult && (
          <div className="w-full mt-4 p-4 rounded-2xl bg-slate-800/95 border border-emerald-500/30 text-left space-y-3 animate-in fade-in zoom-in-95 shadow-xl">
            {/* 標題與金額 */}
            <div className="flex items-center justify-between border-b border-slate-700/80 pb-2.5">
              <div className="flex items-center gap-1.5 flex-wrap">
                <input
                  ...
                  className="font-bold text-sm text-white bg-slate-900/60 border border-slate-700 rounded-lg px-2 py-1 outline-none focus:ring-1 focus:ring-emerald-400"
                />
                {parsedResult.engineType === 'local_zero_token' ? (
                  <span className="text-[10px] bg-emerald-950/80 text-emerald-300 border border-emerald-700/80 px-1.5 py-0.5 rounded-full font-bold flex items-center gap-0.5">
                    <Zap className="w-2.5 h-2.5 text-emerald-400" />
                    0 Token 秒級
                  </span>
                ) : (
                  <span className="text-[10px] bg-teal-950/80 text-teal-300 border border-teal-700/80 px-1.5 py-0.5 rounded-full font-bold flex items-center gap-0.5">
              ...
              <div className="flex items-center gap-1">
                <span className="text-xs text-slate-400 font-bold">NT$</span>
                <input
                  type="number"
                  value={parsedResult.amount === 0 ? '' : parsedResult.amount}
                  placeholder="0"
                  onChange={(e) => setParsedResult({ ...parsedResult, amount: Number(e.target.value) || 0 })}
                  className="w-24 font-extrabold text-base text-emerald-400 font-mono bg-slate-900/60 border border-slate-700 rounded-lg px-2 py-1 outline-none focus:ring-1 focus:ring-emerald-400 text-right"
                />
              </div>
            </div>
            ...
            <div className="space-y-1.5 pt-1">
              <span className="text-[11px] font-bold text-slate-400 block">歸屬標籤 (單一標籤)：</span>
              <div className="flex flex-wrap gap-1.5 max-h-24 overflow-y-auto pr-1">
                {currentTags.map((t) => {
                  const isSelected = (parsedResult.tags?.[0] || '未歸類') === t;
                  return (
                    <button
                      key={t}
                      type="button"
                      onClick={() => {
                        setParsedResult({ ...parsedResult, tags: [t] });
                      }}
                      className={`text-[11px] px-2.5 py-1 rounded-xl font-medium transition border ${
                        isSelected
        ...
        {!parsedResult && !isParsing && (
          <div className="mt-4 pt-3 border-t border-slate-800 w-full text-left space-y-2">
            <p className="text-[11px] font-bold text-slate-400 flex items-center gap-1">
              <Zap className="w-3.5 h-3.5 text-emerald-400" />
              <span>您可以這樣說 (0 Token 秒級自動分類)：</span>
            </p>
